...
const IMAGING_PROMPT = `识别这张影像检查报告（B超/CT/MRI/肠镜/胃镜等）的检查日期和诊断结论。

返回JSON格式：
{"date": "YYYY-MM-DD", "conclusion": "诊断结论原文"}

要求：
1. date: 提取报告中的检查日期，格式为YYYY-MM-DD，如找不到则返回空字符串
2. conclusion: 原样提取报告中"诊断结论"/"诊断意见"/"检查结论"部分的文字，不要总结或改写
...
export interface ImagingRecognitionResult {
  date: string; // YYYY-MM-DD or empty
  conclusion: string;
}

export async function recognizeImagingReport(
  imageFilePath: string
): Promise<ImagingRecognitionResult> {
  try {
    const base64 = await imageToBase64(imageFilePath);
    const imageUrl = `data:image/jpeg;base64,${base64}`;
